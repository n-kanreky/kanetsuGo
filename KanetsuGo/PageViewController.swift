...
class PageViewController: UIPageViewController, UIPageViewControllerDataSource {
    ...
    func pageViewController(_ pageViewController: UIPageViewController, viewControllerBefore viewController: UIViewController) -> UIViewController? {
        ...
        if viewController.isKind(of: WT5ViewController.self)// 現在のviewControllerがThirdViewControllerかどうか調べる
        {
            // 3 -> 2
            return getFourth()
            
        } else if viewController.isKind(of: WT4ViewController.self) {// 現在のviewControllerがSecondViewControllerかどうか調べる
            // 2 -> 1
            return getThird()
        } else if viewController.isKind(of: WT3ViewController.self) {// 現在のviewControllerがSecondViewControllerかどうか調べる
            // 2 -> 1
            return getSecond()
        } else if viewController.isKind(of: WT2ViewController.self) {// 現在のviewControllerがSecondViewControllerかどうか調べる
            // 2 -> 1
            return getSecond()
        } else {
            // 1 -> end of the road
